...
public:
    ...
    optional<vector<Document>> FindTopDocuments(const string& raw_query,
        ...
        return make_optional(matched_documents);
    }
...
int main() {
    ...
    // Явно игнорируем результат метода AddDocument, чтобы избежать предупреждения о неиспользуемом результате его вызова
