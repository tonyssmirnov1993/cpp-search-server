...
vector<pair<int, int>> FindAllDocuments(const vector<pair<int, vector<string>>>& documents,
    const set<string>& query_words) // Превратите функцию FindDocuments в FindAllDocuments
{
    vector<pair<int, int>> matched_documents;
    for (const auto& document : documents) {
        const int relevance = MatchDocument(document, query_words);
...
vector<pair<int, int>> FindTopDocuments(const vector<pair<int, vector<string>>>& documents,
    ...
    // Вектор пар matched_documents содержит пары {релевантность, id}
    // FindTopDocuments должна вернуть пары {релевантность, id}
