...
private:
    ...
    bool ValidQuery(const Query& query) const {
        for (const string& word : query.plus_words) {
            if ((IsValidWord(word) == false) || (word[word.size() - 1] == '-')){
                return false;
            }
        }
        
        for (const string& word : query.minus_words) {
            if ((word == "") || ((!IsValidWord(word) == false) || (word[1] == '-')) || (word[word.size() - 1] == '-')){
                return false;                            
            } 
        }
        return true;
...
int main() {
    ...
    if (search_server.FindTopDocuments("asdw-"s, documents)) {
        for (const Document& document : documents) {
            PrintDocument(document);
        }
    }
    ...
    if (search_server.FindTopDocuments("word - "s, documents)) {
        for (const Document& document : documents) {
            PrintDocument(document);
        }
    }
