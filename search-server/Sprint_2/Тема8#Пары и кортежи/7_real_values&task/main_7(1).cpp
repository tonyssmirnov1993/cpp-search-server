...
public:
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        for (const string& plus_word : parsed_query.plus_words) {
           if (word_to_document_freqs_.count(plus_word)) {
               if (word_to_document_freqs_.at(plus_word).count(document_id)) {
                   if (count(matched_words.begin(), matched_words.end(), plus_word) == 0) {
        ...
        for (const string& minus_word : parsed_query.minus_words) {
...
int main() {
 SearchServer search_server;
    search_server.AddDocument(0, "белый кот и модный ошейник",        DocumentStatus::ACTUAL, {1, 2, 3, 4, 5});
    search_server.AddDocument(1, "пушистый кот пушистый хвост",       DocumentStatus::ACTUAL, {-1, -2, 30, -3, 44, 5});
    search_server.AddDocument(2, "ухоженный пёс выразительные глаза", DocumentStatus::ACTUAL, {12, -20, 80, 0, 8, 0, 0, 9, 67});
   
    for (Document& document : search_server.FindTopDocuments("пушистый и ухоженный кот", DocumentStatus::ACTUAL)) {
